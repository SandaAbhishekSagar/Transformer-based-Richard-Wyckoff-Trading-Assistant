...
def add_technical_indicators(data):
    ...
    try:
        ...
        #Simple moving averages calculate the average price over a specified period 
        ...
        # Calculate RSI -Relative Strength Index measures and speed and change of price movements
        df['RSI'] = 100 - (100 / (1 + rs))
        
        # Calculate Bollinger Bands- bands expand during volatile periods and contract during less volatile periods.

        # In the Wyckoff method, these bands help identify springs and upthrusts
        df['BB_Middle'] = df['MA20']
        df['BB_Std'] = df['Close'].rolling(window=20, min_periods=1).std()
        df['BB_Upper'] = df['BB_Middle'] + (df['BB_Std'] * 2)#overbought condition
        df['BB_Lower'] = df['BB_Middle'] - (df['BB_Std'] * 2)#oversold condition
        ...
        for i in range(1, len(df)):
            if df['Close'].iloc[i] > df['Close'].iloc[i-1]:
                obv += df['Volume'].iloc[i] #adds volume for updays
            elif df['Close'].iloc[i] < df['Close'].iloc[i-1]:
                obv -= df['Volume'].iloc[i] #subtracts volume on down days
            df.iloc[i, df.columns.get_loc('OBV')] = obv
        
        # Fill NaN values to avoid issues in further calculations
        # df = df.fillna(0)
